...
```text
DeepSeek V3 │ Context 88% left │ tokens 73.2k↑ 3.3k↓ │ cache 92.2% │ balance ¥38.50
```

GLM：

```text
GLM-5 │ Context 88% left │ tokens 73.2k↑ 3.3k↓ │ cache 92.2% │ 5h 100% left · week 59% left · reset 07-12 18:00
```
...
- `Context`：当前上下文剩余比例
- `left`：对应项目的剩余比例
- `tokens`：会话累计输入和输出 Token
- `↑`：会话累计输入 Token，包含普通输入、缓存创建和缓存读取 Token
- `↓`：会话累计输出 Token
- `cache`：缓存读取 Token 占全部输入 Token 的比例；全部输入包含普通输入、缓存创建和缓存读取 Token
